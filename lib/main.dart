...
class _ChatScreenState extends State<ChatScreen> {
  ...
  String _selectedModelString = 'gemini-1.5-flash'; 
  bool _isLoading = false;
  bool _showSettings = true;

  final List<Map<String, String>> _modelList = [
    {'name': 'Gemini 1.5 Flash (Riset Cepat)', 'code': 'gemini-1.5-flash'},
    {'name': 'Gemini 1.5 Pro (Riset Mendalam)', 'code': 'gemini-1.5-pro'},
  ...
  Future<void> _loadSavedData() async {
    ...
    setState(() {
      ...
      _selectedModelString = prefs.getString('selected_model') ?? 'gemini-1.5-flash';
      ...
      if (historyJson != null) {
        try {
          ...
          _messages.clear();
          _messages.addAll(decoded.map((e) => ChatMessage.fromJson(e as Map<String, dynamic>)).toList());
        } catch (_) {
          _messages.clear();
        }
      }
  ...
  void _clearHistory() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('🗑️ Riwayat telah dibersihkan!')));
  }
  ...
  void _sendMessage() async {
    ...
    if (apiKey.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('⚠️ Mohon isi API Key!'), backgroundColor: Colors.orange));
      return;
    }
    ...
    try {
      ...
      final history = _messages
          ...
          .map((msg) => Content(msg.isUser ? 'user' : 'model', [TextPart(msg.text)]))
          .toList();
      ...
      appBar: AppBar(
        ...
        actions: [
          DropdownButton<String>(
            ...
            items: _modelList.map((model) => DropdownMenuItem<String>(value: model['code'], child: Text(model['name']!))).toList(),
            onChanged: (String? newValue) async {
              if (newValue != null) {
                final prefs = await SharedPreferences.getInstance();
                setState(() => _selectedModelString = newValue);
                await prefs.setString('selected_model', newValue);
              }
            },
          ),
          IconButton(icon: const Icon(Icons.delete_sweep, color: Colors.redAccent), onPressed: _clearHistory),
          IconButton(icon: Icon(_showSettings ? Icons.keyboard_arrow_up : Icons.vpn_key), onPressed: () => setState(() => _showSettings = !_showSettings))
        ],
      ),
      body: Column(
        children: [
                ...
                children: [
                  Expanded(
                    child: TextField(
                      ...
                      decoration: const InputDecoration(labelText: 'Masukkan Gemini API Key', border: OutlineInputBorder(), isDense: true),
                      obscureText: true,
                    ),
                  ),
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () async {
                      if (_apiKeyController.text.trim().isNotEmpty) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('✅ API Key disimpan!')));
                      }
                    },
          ...
          Expanded(
            child: _messages.isEmpty
                ? Center(child: Text('Mulai Deep Research dengan Kajimini AI', style: TextStyle(color: Colors.grey[500])))
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Align(
                        ...
                        child: Container(
                          ...
                          child: isUser
                              ? SelectableText(message.text, style: const TextStyle(color: Colors.white, fontSize: 16))
                              : MarkdownBody(
                                  data: message.text,
                                  selectable: true,
                                  styleSheet: MarkdownStyleSheet.fromTheme(Theme.of(context)).copyWith(
                                    p: const TextStyle(color: Colors.white, fontSize: 15, height: 1.4),
                                    code: const TextStyle(color: Colors.orangeAccent, backgroundColor: Colors.black38, fontFamily: 'monospace', fontSize: 13),
                                  ),
                                  builders: {'codeblock': MarkdownCodeBlockBuilder()},
                                ),
                        ),
                      );
                    },
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Row(
              children: [
                Text(widget.language, style: const TextStyle(color: Colors.white54, fontSize: 12, fontFamily: 'monospace')),
                const Spacer(),
                InkWell(
                  onTap: () {
                  ...
                  child: Row(
                    children: [
                      Icon(_copied ? Icons.check : Icons.copy, size: 14, color: _copied ? Colors.green : Colors.white54),
                      const SizedBox(width: 4),
                      Text(_copied ? 'Disalin' : 'Salin', style: TextStyle(color: _copied ? Colors.green : Colors.white54, fontSize: 12)),
                    ],
                  ),
                ),
              ],
          ...
          SingleChildScrollView(scrollDirection: Axis.horizontal, padding: const EdgeInsets.all(12), child: SelectableText(widget.code, style: const TextStyle(fontFamily: 'monospace', fontSize: 13, color: Colors.lightBlueAccent))),
        ],
      ),
    );
  }
